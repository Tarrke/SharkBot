...
async function login() {
    ...
    try {
        axios.post(BASE_URL + '/auth/signin', qs.stringify({
...
async function call_api(endpoint, token) {
    ...
    try {
        let response = await axios.post(BASE_URL + endpoint, 
            {
                allycodes: [413347731]
            }, 
            { headers: {
...
async function loginv2() {
    ...
    try {
        ...
        if( endOfLife.getTime() > Date.now() ) {
            ...
            try {
                let response = await axios.post(BASE_URL + '/auth/signin', qs.stringify({
                    'username': process.env.SWGOHAPI_USER,
                    'password': process.env.SWGOHAPI_PASS,
                    'grant_type': 'password',
                    'client_id': process.env.SWGOHAPI_CLIENTID,
